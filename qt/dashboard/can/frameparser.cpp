...
FrameParser::FrameParser(QObject* p_racev = 0)
...
#else
	// show every data byte
	cout << __func__ << ":" << __LINE__ << " seq " << seq_num << " ";
	for ( i = 0; i < NUM_BYTE_FRAME; i++ ) {
	    m_pInfo->probe_temperature[( seq_num - 1 )*NUM_BYTE_FRAME + i]
		= pframe->payload().at(i+1);
	    // memcpy(m_pInfo->probe_temperature[( seq_num - 1 )*NUM_TPROBE + i],
	    cout << "0x" << setw(2) << hex << (pframe->payload()[i+1]/*&0xFF*/) << " ";
	}
...
bool FrameParser::isAtLeastXBytesGreaterThanCriteria(
	...
	std::cout << __func__ << ":" <<  __LINE__ << ex.what() << endl;
    }
...
int FrameParser::parse(QCanBusFrame* pframe) {
	...
	if ( fid == J1939_BAM_HEAD || fid == J1939_BAM_BODY ) {
...
#if defined ( EN_IPC_ )
	    p_racev1->m_CANBusFrameQueueMutex.lock();
	    if ( m_init != 0 ) {
		p_racev1->m_CANBusFrameQueueWaitCondition
		    .wait(&p_racev1->m_CANBusFrameQueueMutex);
		m_init = 1;
		cout<<__FILE__ << ":" << __LINE__<< "latch" <<endl;
	    }
#endif
	    // qDebug() << __func__ << ":" << __LINE__;
	    switch ( fid ) {
		...
		break;
		case VCU_HMI_Msg_4:
		    m_pInfo->FrameVCU_HMI_MSG4 = *pframe;
		    emit canSignalFrame(fid, pframe);
		break;
		case BMS_VCU_Msg01:
		...
		break;
		case VCU_DMRT_CLR:
		    m_pInfo->FrameVCU_DMRT_CLR = *pframe;
		    emit canSignalFrame(fid, pframe);
		break;
		case DCDC_MSG00:
		...
		default:
#if 1
		    if ( pframe->frameType() != 1 ) {
			std::cout << __func__ << ":" <<  __LINE__ << " "
		    << "fid  0x" << setw(8) << hex << fid << std::dec << " "
...
#if defined ( EN_IPC_ )
	    p_racev1->m_CANBusFrameQueueMutex.unlock();
	    p_racev1->m_CANBusFrameQueueWaitCondition.wakeAll();
	    //qDebug() << __FILE__ << ":" << __LINE__ << "release-";
#endif
	    rc = 1;
	}
	p_racev1->m_CANBusCircularBufferTail++; // finished
	if ( NUM_CANCB <= p_racev1->m_CANBusCircularBufferTail ) {
	    //std::cout << __func__ << ":" <<  __LINE__ << " " << p_racev1->m_CANBusCircularBufferTail << endl;
	    p_racev1->m_CANBusCircularBufferTail = 0;
	    ...
	    p_racev1->m_CANBusCircularBuffer[NUM_CANCB-1].setFrameId(0);
	}
	else {
	    p_racev1
		->m_CANBusCircularBuffer[p_racev1->m_CANBusCircularBufferTail-1]
		.setFrameId(0);
	}
    } catch (const std::exception& ex) {
	std::cout << __func__ << ":" <<  __LINE__ << ex.what() << endl;
...
#if 0
//#define ENUMERATE_KEY_VALUE_
#if defined ( ENUMERATE_KEY_VALUE_ )
    // TODO: validate after changing from QQueue to QVector
    qDebug() << "-------";
    QVector<QCanBusFrame*> Q1
    = p_racev1->m_CANBusFrameBins[p_racev1->m_pInfo->m_addrVCU_HMI_MSG1];
    QVector<QCanBusFrame*>::ConstIterator iter = Q1.constBegin();
    for ( ; iter != Q1.constEnd(); ++iter ) {
	const QCanBusFrame* pframe = *iter;
        qDebug() << QString("0x%1")
        .arg(pframe->frameId(), 8, 16, QLatin1Char('0'))
        << " : " << pframe->payload().toHex(0);
    }
    qDebug() << "+++++++";
#endif
#endif
